...
// Package llmCheatSemanticWebIntegrity is one of the 7 Anti-Cheat category probes. It is a thin
...
package llmCheatSemanticWebIntegrity
...
const Probe = "llmCheatSemanticWebIntegrity"
...
const category = "semantic-web-integrity"
...
func Run(raw *checker.RawResults) ([]finding.Finding, string, error) {
	...
	if len(findings) == 0 {
		f, err := finding.NewTrue(fs, Probe, "no semantic-web-integrity patterns detected", nil)
		if err != nil {
			return nil, Probe, fmt.Errorf("create finding: %w", err)
		}
		findings = append(findings, *f)
