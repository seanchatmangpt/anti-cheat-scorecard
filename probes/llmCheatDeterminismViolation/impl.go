...
// Package llmCheatDeterminismViolation is one of the 7 Anti-Cheat category probes. It is a thin
...
package llmCheatDeterminismViolation
...
const Probe = "llmCheatDeterminismViolation"
...
const category = "determinism-and-provenance-violation"
...
func Run(raw *checker.RawResults) ([]finding.Finding, string, error) {
	...
	if len(findings) == 0 {
		f, err := finding.NewTrue(fs, Probe, "no determinism-and-provenance-violation patterns detected", nil)
		if err != nil {
			return nil, Probe, fmt.Errorf("create finding: %w", err)
		}
		findings = append(findings, *f)
