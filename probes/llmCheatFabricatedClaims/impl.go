...
// Package llmCheatFabricatedClaims is one of the 7 Anti-Cheat category probes. It is a thin
...
const Probe = "llmCheatFabricatedClaims"
...
func Run(raw *checker.RawResults) ([]finding.Finding, string, error) {
	...
	if len(findings) == 0 {
		f, err := finding.NewTrue(fs, Probe, "no fabricated-claims patterns detected", nil)
		if err != nil {
			return nil, Probe, fmt.Errorf("create finding: %w", err)
		}
		findings = append(findings, *f)
