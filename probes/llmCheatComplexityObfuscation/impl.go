...
// Package llmCheatComplexityObfuscation is one of the 7 Anti-Cheat category probes. It is a thin
...
package llmCheatComplexityObfuscation
...
const Probe = "llmCheatComplexityObfuscation"
...
const category = "complexity-and-surface-obfuscation"
...
func Run(raw *checker.RawResults) ([]finding.Finding, string, error) {
	...
	if len(findings) == 0 {
		f, err := finding.NewTrue(fs, Probe, "no complexity-and-surface-obfuscation patterns detected", nil)
		if err != nil {
			return nil, Probe, fmt.Errorf("create finding: %w", err)
		}
		findings = append(findings, *f)
